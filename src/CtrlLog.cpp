...
//------------------------------------------------------------------------------
CCtrlLog* CCtrlLog::Create (wxTextCtrl* LogZone)
{
	if (!PtCtrlLog) PtCtrlLog = new CCtrlLog( LogZone );
	return PtCtrlLog;
}
//------------------------------------------------------------------------------
void CCtrlLog::Kill()
...
//------------------------------------------------------------------------------
CCtrlLog::CCtrlLog(wxTextCtrl* LogZone)
{
	FenLog = LogZone;
	...
	)
{
	...
	if (FenLog)
	{
		if (FenLog->GetNumberOfLines()> NbLigneMax )
		{
			lon=FenLog->GetLineLength(0);
			FenLog->Remove(0, lon+2);
		}
		FenLog->SetDefaultStyle(wxTextAttr(Coul));
		FenLog->SetInsertionPointEnd();
		FenLog->WriteText(Texte + _T("\n"));
	}
	return 0;
}
...
void CCtrlLog::SetLogZone(wxTextCtrl* LogZone)
{
	FenLog=LogZone;
}
